...
Resource.VDAS.dmaTimeout = 100000;
...
SeqControl(5).condition = 'Trigger_1_Falling'; % input BNC #1 falling edge
SeqControl(5).argument = 0; % 500 msec timeout delay
SeqControl(6).command = 'sync';
nsc = 7; % nsc is count of SeqControl objects

n = 1; % n is count of Events

Event(n).info = 'Wait for triggerIn';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0; %calling waitForPosition function
Event(n).seqControl = [5,6]; %Syncronising so that hwardware sequence waits for position increment
...
save('MatFiles/L11-5vFlash_TriggerIn_2s');
return
