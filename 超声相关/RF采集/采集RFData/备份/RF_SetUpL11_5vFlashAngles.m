...
%   for its use
%
% File name: SetUpL11_5vFlashAngles.m - Example of plane wave imaing with
...
na = 3;      % Set na = number of angles.
if (na > 1), dtheta = (36*pi/180)/(na-1); P.startAngle = -36*pi/180/2; 
...
% Trans.Serial = serialport('com3',9600);
...
Resource.RcvBuffer(1).rowsPerFrame = na*4000; % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 50;    % 30 frames stored in RcvBuffer.
...
Process(3).method = 'trigger';
Process(3).Parameters = {};

                     
% Specify SeqControl structure arrays. %~ 3角度平面波复合成像的1帧耗时为 83*2+334 = 500us，采样帧率为2kHz
SeqControl(1).command = 'jump'; % jump back to start
SeqControl(1).argument = 1;
SeqControl(2).command = 'timeToNextAcq';  % time between synthetic aperture acquisitions
SeqControl(2).argument = 200;  % 83 usec
SeqControl(3).command = 'timeToNextAcq';  % time between frames
SeqControl(3).argument = 500;  % 334 usec
...
% 发送trigger到arduino
%     if i==1 || i==Resource.RcvBuffer(1).numFrames
%         Event(n).info = 'trigger';
%         Event(n).tx = 0;
%         Event(n).rcv = 0;
%         Event(n).recon = 0;
...
%~ 调用外部函数
EF(2).Function = vsv.seq.function.ExFunctionDef('trigger',@trigger);

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 5;

% Save all the structures to a .mat file.
save('MatFiles/L11-5vFlashAngles');
return
