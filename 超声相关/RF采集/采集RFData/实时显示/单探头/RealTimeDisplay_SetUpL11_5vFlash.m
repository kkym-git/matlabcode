...
Resource.RcvBuffer(1).rowsPerFrame = 4000;   % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 200;       % 100 frames used for RF cineloop.
...
SeqControl(2).argument = 500;  % 3 msec
SeqControl(3).command = 'returnToMatlab';
SeqControl(4).command = 'triggerOut';
% SeqControl(4).condition = 'syncSYNC_CLK';
% SeqControl(4).argument = 100;
...
%     else
%         Event(n).seqControl = [2,nsc]; % modify last acquisition Event's seqControl
%     end 
...
save('MatFiles/RealTimeDisplay_L11-5vFlash');
return
