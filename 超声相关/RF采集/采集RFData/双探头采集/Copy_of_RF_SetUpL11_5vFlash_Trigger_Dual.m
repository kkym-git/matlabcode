...
%~ 2023-03-09
%~ 修改VsUpdate.m的底层代码（1130~1137行），给ReconInfo.Aperture属性赋值
...
% Trans.ElementPos = zeros(Trans.numelements,5); %~ 用于图像重建的参数
Trans.ElementPos = [Trans.ElementPos;Trans.ElementPos];
...
PData(1).Size(2) = ceil((Trans.numelements/2*Trans.spacing)/PData(1).PDelta(1));
PData(1).Size(3) = 1;      % single image page
PData(1).Origin = [-Trans.spacing*(Trans.numelements/2-1)/2,0,P.startDepth]; % x,y,z of upper lft crnr.
% No PData.Region specified, so a default Region for the entire PData array will be created by computeRegions.
PData(2) = PData(1);
PData(2).Origin = [-Trans.spacing*(Trans.numelements/2-1)/2,0,P.startDepth];
...
Resource.RcvBuffer(1).numFrames = 10;       %~ 采样帧率2000 frames/s。总共采集（na*numFrames）帧的数据。na帧组成一个大数据帧。numFrames表示大帧的数目。
...
Resource.DisplayWindow(2).Position = [600,(ScrnSize(4)-(DwHeight+150))/2, ...  % lower left corner position
                                      DwWidth, DwHeight];
...
maxAcqLength = ceil(sqrt(P.endDepth^2 + ((Trans.numelements/2-1)*Trans.spacing)^2)); %~ 对应每帧2304个点
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Reconstruct & display L11-5v(1)';
    ...
    Event(n).info = 'Reconstruct & display L11-5v(2)';
    Event(n).tx = 0;         % no transmit
    Event(n).rcv = 0;        % no rcv
...
save('MatFiles/L11-5vFlash_Trigger_Dual');
return
